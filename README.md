...
- **채널 +** 로 새 채널/프로젝트 생성, 채널 행의 **⋯** 로 이름·주제 수정과 삭제
- **멤버 +** 로 에이전트 추가 (기존 선택 · 역할 템플릿 8종으로 새로 만들기),
  멤버 행 클릭으로 역할·응답조건·활성 상태 수정과 삭제, **×** 로 채널에서만 빼기
...
| PATCH | `/api/agents/{id}` | 에이전트 수정 (부분 갱신) |
| DELETE | `/api/agents/{id}` | 에이전트 삭제 (과거 발언은 보존) |
| PATCH | `/api/channels/{id}` | 채널 이름·주제 수정 |
| DELETE | `/api/channels/{id}?confirm=<이름>` | 채널 삭제 (연쇄) |
...
| 대상 | 지우는 것 | 남기는 것 |
|---|---|---|
| 에이전트 | 에이전트, 모든 채널 소속 | **과거 발언 전부** |
| 채널 | 메시지·태스크·요약·실행기록·멤버십 | 에이전트 자체 (다른 채널에서 계속 씀) |

에이전트를 지워도 발언이 남는 이유: 메시지는 append-only 이벤트 로그이고,
지우면 히스토리가 다시 쓰여진다. `author_name` 을 메시지에 비정규화해둔 것이
이걸 위해서다 — 삭제된 에이전트의 발언도 이름과 함께 그대로 렌더링된다.

채널 삭제는 되돌릴 수 없으므로 API 가 `?confirm=<채널이름>` 을 요구하고,
UI 도 이름을 직접 입력해야 실행된다.

## 에이전트의 외부 접근

초기에는 소통·태스크 도구만 있어서 리서처가 이렇게 답했다:

> "제 환경에 웹/뉴스 접근 도구 자체가 없어서 '오늘자'는 물리적으로 못 가져옵니다
> — 이건 게으름이 아니라 도구 부재입니다."

두 갈래로 열었다.

| 능력 | 어디서 오나 | provider 교체 시 |
|---|---|---|
| **파일 읽기** `list_files`·`read_file`·`describe_table` | 우리 레지스트리 (`core/files.py`) | **유지됨** |
| **페이지 읽기** `fetch_url` | 우리 레지스트리 (`core/web.py`) | **유지됨** |
| **웹 검색** | `claude -p` 내장 `WebSearch` | 사라짐 — 사내 AI 는 자체 검색이 필요 |

### 산출물 생성

에이전트가 파일을 만들어 채널에 올린다. 업로드된 파일과 똑같이 취급되므로
사람이 내려받고, **다른 에이전트가 `read_file` 로 읽어 이어서 작업한다.**

| 도구 | 만드는 것 | 입력 |
|---|---|---|
| `write_file` | MD·TXT·CSV·코드 | 내용 그대로 |
| `create_document` | **.docx** | 마크다운 (`#` `-` `\|표\|` ```` ``` ````) |
| `create_spreadsheet` | **.xlsx** | CSV 텍스트 |
| `create_slides` | **.pptx** | 마크다운 (`#` 가 새 슬라이드) |

**입력을 전부 평평한 문자열로 받는 게 설계의 핵심이다.** 에이전트는 ReAct
액션 블록 안의 JSON 으로 인자를 넘기는데, 중첩 구조(슬라이드 배열 안의 불릿
배열…)는 이스케이프가 얽혀 자주 깨진다. 마크다운·CSV 는 모델이 이미 잘 쓰는
형식이라 실패율이 낮고 사람이 검토하기도 쉽다.

실측: "payments.csv 분석 결과로 엑셀·슬라이드·워드 보고서 만들어주세요" 한
마디에 데이터분석가가 3종을 생성했다 — 19행 7열 엑셀, 7장 슬라이드
(현황→원인→배제된 원인→조치→한계 구성), 표 4개가 든 27문단 워드 보고서.

### 파일 업로드

입력창의 📎 또는 드래그&드롭. 업로드는 **채널에 메시지로 알린다** — 같이 적은
글에 `@데이터분석가` 가 있으면 기존 디스패처가 그를 깨운다. 파일 전용 알림
경로를 따로 만들지 않았다.

`describe_table` 이 핵심이다. 분석가에게 필요한 건 원문 4,000자가 아니라
**컬럼이 뭐고 몇 행이고 각 컬럼에 어떤 값이 들어있는가** 다:

```
구분자: ',' · 2,880행 · 7개 컬럼
컬럼:
  1. pg사 — 문자 고유3개 (예: A페이, B페이, C페이)
  4. 금액 — 숫자 1000~299987
  6. 에러코드 — 문자 고유3개 (예: TIMEOUT_3DS, …) · 빈값 2,740
```

실측: 2,880행 결제 로그를 올리고 "실패가 어디에 몰려 있는지" 물었더니
`list_files → describe_table → read_file(offset 4회)` 를 거쳐
**"급증 시점 8/21 14시, A페이 TIMEOUT_3DS 89건이 증분의 대부분"** 이라고
정확히 짚었다 (데이터에 심어둔 패턴과 일치). 없는 컬럼은 "미분석"이라고 밝혔다.

경로 탈출 방어: 사용자가 준 파일명을 경로로 쓰지 않는다. 디스크에는 생성한
`file_id` 로만 저장하고 원본 이름은 표시용 메타데이터로만 남긴다.

`fetch_url` 은 SSRF 를 막는다. 에이전트가 URL 을 만들어내므로, 클라우드
메타데이터(`169.254.169.254`)나 사내망 주소를 읽어 채널에 게시하면 그게 곧
유출이다. 사설·루프백·링크로컬·예약 대역을 모두 차단한다.

**내장 `WebFetch` 는 일부러 열지 않았다.** 열어봤더니 에이전트가 그쪽을 쓰면서
우리 SSRF 가드와 툴 로그를 통째로 우회했다 (`fetch_url` 호출 0건). 검색은
API 키 없이 우리가 못 만드는 능력이라 내장을 쓰고, 읽기는 우리 것을 쓴다.

`Bash`/`Write`/`Edit` 는 설정에 넣어도 코드에서 걸러낸다 — 에이전트가 만든
텍스트가 곧 셸 명령이 되는 구조라 프롬프트 인젝션에 그대로 노출된다.
